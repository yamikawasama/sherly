...
const App = {
  ...
  applyLoadingImg(){const imgUrl=Store.getLoadingImg();const size=Store.getLoadingImgSize();const l=document.querySelector('.bunny-loader');if(imgUrl&&l){l.innerHTML=`<img src="${imgUrl}" style="max-width:${Math.round(200*(size/100))}px;max-height:${Math.round(200*(size/100))}px;object-fit:contain;border-radius:12px;">`;l.style.width='auto';l.style.height='auto';}},
  ...
  renderOrder(){
    ...
    c.innerHTML=`${orderBanner?`<div class="promo-banner animate-fade-in-up" style="margin-bottom:20px;max-height:${Math.round(180*(obSize/100))}px;min-height:${Math.round(80*(obSize/100))}px;"><img src="${orderBanner}" alt="" style="max-height:${Math.round(180*(obSize/100))}px;border-radius:var(--radius-lg);"></div>`:''}
      ...
      <h3 style="margin:20px 0 12px;" class="animate-fade-in-up">💎 แพ็คกระดุม</h3>
  ...
  recalcCart(){
    ...
    ${rows.map(r=>`<div style="display:flex;align-items:center;justify-content:space-between;padding:10px 0;border-bottom:1px solid var(--border-color);">
      <div><div style="font-weight:600;">${r.isSkin?'✨':'💎'} ${r.name} (${r.type==='private'?'พรีไว':r.type==='send'?'ส่ง':'ปกติ'})${r.discLabel}</div><div style="font-size:0.8rem;color:var(--text-secondary);">${r.totalEcho} กระดุม${r.breakdownText?' (สรุป: '+r.breakdownText+')':''}</div></div>
  ...
  // ========== GIFT ==========
  renderGift(){
    const c=document.getElementById('page-gift');const skins=Store.getSkinPacks().filter(s=>s.active);
  ...
  _addProduct(){const products=Store.getProducts();products.push({id:Date.now(),name:'สินค้าใหม่',echoes:0,bonus:0,totalEcho:0,privatePrice:0,normalPrice:0,normalEnabled:false,cost:0,image:'',emoji:'💎'});Store.setProducts(products);this.showToast('✅ เพิ่มสินค้า!');this.renderAdmin();},
  ...
  _adminSkins(ac){
    ...
    ac.innerHTML=`<div class="card"><h3 style="margin-bottom:16px;">🎁 จัดการแพ็คสกิน</h3>
      ...
      ${skins.map((s,i)=>{const calc=this._autoCalcSkinPack(s.targetEcho||0);
        return`<div style="padding:12px;border:1px solid var(--border-color);border-radius:var(--radius-md);margin-bottom:8px;"><div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><strong>${s.emoji} ${s.name}</strong><div style="display:flex;gap:4px;"><span class="badge ${s.active?'badge-green':'badge-red'}">${s.active?'เปิด':'ปิด'}</span><button class="btn-copy" onclick="App._toggleSkin(${i})">${s.active?'🔴':'🟢'}</button></div></div>
  ...
  _addSkinPack(){const skins=Store.getSkinPacks();skins.push({id:Date.now(),name:'แพ็คใหม่',targetEcho:0,sendPrice:0,normalEnabled:false,cost:0,image:'',emoji:'🎁',active:true,customNormalPrice:null,customPrivatePrice:null});Store.setSkinPacks(skins);this.showToast('✅ เพิ่มแพ็คสกิน!');this.renderAdmin();},
  ...
  ac.innerHTML=`<div class="card" style="margin-bottom:20px;"><h3 style="margin-bottom:16px;">📋 คิวเติม (${topup.length})</h3>${this._renderQueueTable(topup)}</div>
    ...
    ${bookings.length>0?`<div class="table-container"><table class="table"><thead><tr><th>ไอดี</th><th>ผู้เช่า</th><th>วัน</th><th>เวลา</th><th>ราคา</th><th>สถานะ</th><th></th></tr></thead><tbody>${bookings.slice().reverse().map(b=>{const r=rentals.find(x=>x.id===b.rentalId);return`<tr><td>${r?.name||'-'}</td><td>${b.userName}</td><td>${b.date}</td><td>${b.startHour}:00-${b.endHour}:00</td><td style="font-weight:600;color:var(--primary);">฿${b.totalPrice}</td><td><span class="status-badge status-${b.status==='done'?'done':b.status==='playing'?'processing':'waiting'}">${b.status==='booked'?'⏳ รอ':b.status==='playing'?'🎮 เล่นอยู่':b.status==='done'?'✅ เสร็จ':'❌ ยกเลิก'}</span></td><td><select class="form-select" style="padding:6px;font-size:0.8rem;width:auto;" onchange="App._updateBookingStatus(${b.id},this.value)"><option value="booked" ${b.status==='booked'?'selected':''}>⏳ รอ</option><option value="playing" ${b.status==='playing'?'selected':''}>🎮 เล่น</option><option value="done" ${b.status==='done'?'selected':''}>✅ เสร็จ</option><option value="cancelled" ${b.status==='cancelled'?'selected':''}>❌ ยกเลิก</option></select></td></tr>`;}).join('')}</tbody></table></div>`:'<p style="color:var(--text-light);">ไม่มีคิวเช่า</p>'}</div>`;},
  _renderQueueTable(orders){if(orders.length===0)return'<p style="color:var(--text-light);">ไม่มีคิว</p>';return`<div class="table-container"><table class="table"><thead><tr><th>คิว</th><th>ลูกค้า</th><th>รายการ</th><th>ยอด</th><th>สลิป</th><th>สถานะ</th><th></th></tr></thead><tbody>${orders.slice().reverse().map(o=>`<tr><td><strong>#${o.queueNumber}</strong></td><td>${o.gameName}<br><span style="font-size:0.7rem;color:var(--text-light);">UID:${o.uid}</span></td><td>${(o.items||[]).map(i=>`${i.isSkin?'✨':'💎'}${i.name}`).join('<br>')}</td><td style="font-weight:600;color:var(--primary);">฿${(o.totalPrice||0).toLocaleString()}</td><td>${o.slipImage?`<img src="${o.slipImage}" style="width:40px;height:60px;object-fit:cover;border-radius:4px;cursor:pointer;" onclick="App.openLightbox('${o.slipImage}')">`:'-'}</td><td><span class="status-badge status-${o.status}">${o.status==='waiting'?'⏳ รอ':o.status==='processing'?'🔄 ทำ':'✅ เสร็จ'}</span></td><td><select class="form-select" style="padding:6px;font-size:0.8rem;width:auto;" onchange="App._updateOrderStatus(${o.id},this.value)"><option value="waiting" ${o.status==='waiting'?'selected':''}>⏳ รอ</option><option value="processing" ${o.status==='processing'?'selected':''}>🔄 ทำ</option><option value="done" ${o.status==='done'?'selected':''}>✅ เสร็จ</option></select></td></tr>`).join('')}</tbody></table></div>`;},
  ...
  _adminRentals(ac){
    ...
    ac.innerHTML=`<div class="card" style="margin-bottom:20px;"><h3 style="margin-bottom:16px;">🎮 จัดการไอดีเช่า</h3>
      ${rentals.map((r,i)=>`<div style="padding:12px;border:1px solid var(--border-color);border-radius:var(--radius-md);margin-bottom:8px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><strong>${r.emoji} ${r.name}</strong><div style="display:flex;gap:4px;"><span class="badge ${r.active?'badge-green':'badge-red'}">${r.active?'เปิด':'ปิด'}</span><button class="btn-copy" onclick="App._toggleRentalActive(${i})">${r.active?'🔴':'🟢'}</button><button class="btn-copy" onclick="App._deleteRental(${i})">🗑️</button></div></div>
      ...
      ${bookings.length>0?`<div class="table-container"><table class="table"><thead><tr><th>ไอดี</th><th>ผู้เช่า</th><th>วัน</th><th>เวลา</th><th>ราคา</th><th>สถานะ</th><th></th></tr></thead><tbody>${bookings.slice().reverse().map(b=>{const r=rentals.find(x=>x.id===b.rentalId);return`<tr>
        ...
        <td><select class="form-select" style="padding:6px;font-size:0.8rem;width:auto;" onchange="App._updateBookingStatus(${b.id},this.value)"><option value="booked" ${b.status==='booked'?'selected':''}>⏳ รอ</option><option value="playing" ${b.status==='playing'?'selected':''}>🎮 เล่นอยู่</option><option value="done" ${b.status==='done'?'selected':''}>✅ เสร็จ</option><option value="cancelled" ${b.status==='cancelled'?'selected':''}>❌ ยกเลิก</option></select></td>
  ...
  _adminPromos(ac){
    ...
    ac.innerHTML=`<div class="card"><h3 style="margin-bottom:16px;">🎉 จัดการโปรโมชั่น (แสดงหน้าแรก)</h3>
      ${promos.map((p,i)=>`<div style="padding:10px;border:1px solid var(--border-color);border-radius:var(--radius-md);margin-bottom:8px;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;"><strong>${p.title.substring(0,30)}...</strong><div style="display:flex;gap:4px;"><span class="badge ${p.active?'badge-green':'badge-red'}">${p.active?'เปิด':'ปิด'}</span><button class="btn-copy" onclick="App._togglePromo(${i})">${p.active?'🔴':'🟢'}</button><button class="btn-copy" onclick="App._deletePromo(${i})">🗑️</button></div></div>

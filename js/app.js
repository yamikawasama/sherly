...
const App = {
  ...
  _renderQueueTable(orders){if(orders.length===0)return'<p style="color:var(--text-light);">ไม่มีคิว</p>';return`<div class="table-container"><table class="table"><thead><tr><th>คิว</th><th>ลูกค้า</th><th>รายการ</th><th>ยอด</th><th>สลิป</th><th>สถานะ</th><th></th></tr></thead><tbody>${orders.slice().reverse().map(o=>`<tr><td><strong>#${o.queueNumber}</strong></td><td>${o.gameName}<br><span style="font-size:0.7rem;color:var(--text-light);">UID:${o.uid}</span></td><td>${(o.items||[]).map(i=>`${i.isSkin?'✨':'💎'}${i.name}`).join('<br>')}</td><td style="font-weight:600;color:var(--primary);">฿${(o.totalPrice||0).toLocaleString()}</td><td>${o.slipImage?`<img src="${o.slipImage}" style="width:40px;height:60px;object-fit:cover;border-radius:4px;cursor:pointer;" onclick="App.openLightbox('${o.slipImage}')">`:'-'}</td><td><span class="status-badge status-${o.status}">${o.status==='waiting'?'⏳ รอ':o.status==='processing'?'🔄 ทำ':'✅ เสร็จ'}</span></td><td><div style="display:flex;gap:4px;align-items:center;"><select class="form-select" style="padding:6px;font-size:0.8rem;width:auto;" onchange="App._updateOrderStatus(${o.id},this.value)"><option value="waiting" ${o.status==='waiting'?'selected':''}>⏳ รอ</option><option value="processing" ${o.status==='processing'?'selected':''}>🔄 ทำ</option><option value="done" ${o.status==='done'?'selected':''}>✅ เสร็จ</option></select><button class="btn-copy" style="color:#e53e3e;font-size:1rem;" onclick="event.stopPropagation();App._deleteOrder(${o.id})" title="ลบออเดอร์">🗑️</button></div></td></tr>`).join('')}</tbody></table></div>`;},
